...
test.describe("WEB-TC-018: Eliminar producto del carrito", () => {
  test("debe quitar un producto y reflejar el carrito vacio", async ({
    page,
    request,
  }) => {
    ...
    await page.getByTestId("remove-from-cart-1").click();

    await expect(page.getByTestId("cart-item-1")).toHaveCount(0);
    await expect(page.getByTestId("cart-count")).toHaveText("0");
